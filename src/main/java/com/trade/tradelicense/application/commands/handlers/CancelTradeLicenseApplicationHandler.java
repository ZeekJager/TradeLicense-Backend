...
@Service
public class CancelTradeLicenseApplicationHandler implements CommandHandler<CancelTradeLicenseApplicationCommand, Result<TradeLicenseApplication>> {
    private final TradeLicenseApplicationRepositoryPort repository;
    private final SubmitApplicationValidator validator;

    public CancelTradeLicenseApplicationHandler(
        ...
        this.validator = Objects.requireNonNull(validator, "Cancel validator is required");
    }

    @Override
    @Transactional
    public Result<TradeLicenseApplication> handle(CancelTradeLicenseApplicationCommand command) {
        ...
        application.cancel(new Actor(new UserId(command.actorId()), command.role()));
        return Result.success("Trade license application cancelled", repository.save(application));
    }
}
